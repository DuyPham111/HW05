...
**Phép kiểm chứng độc lập (dừng tải, đọc lại RSS):** Lượt Soak kết thúc lúc 19:49:52. Đọc lại RSS sau khi dừng tải, không có request nào tới backend: **31,6 MB (32.316 K, PID 3728)** — **thấp hơn cả** mức lúc bắt đầu lượt (61,5 MB) và thấp hơn nhiều so với mức ổn định trong-lượt (~76 MB). Đây là bằng chứng độc lập mạnh nhất có thể có cho việc không rò rỉ: nếu `userCarts` tích luỹ dữ liệu không thể thu hồi, RSS sau khi dừng tải phải **bằng hoặc cao hơn** mức trong-lượt, chứ không thể **giảm xuống dưới cả baseline ban đầu**. Kết luận: đỉnh 113,7 MB ở phút 1 và mức ổn định ~76 MB trong suốt lượt chạy là bộ nhớ tạm phục vụ request đang xử lý (buffer HTTP, object tạm của `userCarts`/JSON parsing) — hoàn toàn được V8 GC thu hồi khi hết tải, **không phải rò rỉ tích luỹ**.
